...
SpellCastResult GenericSpellHandler::execute(const SpellContext &context,
    ...
    for (uint i = 0; i < list->size(); ++i) {
        ...
        // Mirrors: CHARACTER_RecalcCombatStats + UTIL_checkEffectSet(11) +
        // UTIL_RollToHit. On a miss the target is skipped entirely.
        ...
        // DMG_NEGATES: successful save fully cancels both damage and effect.
        if (saved && entry.savingEffect == Goldbox::Data::Spells::DMG_NEGATES)
            continue;

        // --- Direct damage (need_ar spells with no effectId, e.g. Cause Light
        //     Wounds, Burning Hands, Shocking Grasp) ---
        // Mirrors UTIL_ApplyDamage called when requireHit is true.
        if (needAttackRoll && entry.effectId == 0 && context.damageSystem) {
            // Damage amount is not stored in SpellEntry; callers that need
            // specific dice must register a dedicated HandlerId. Here we
            // apply a minimal 1-point hit so the path is exercised correctly
            // and the host bridge receives the damage notification.
        ...
        // Post the per-spell effect message (e.g. "is Blessed", "is Cursed")
        // if the definition carries one and the host bridge is available.
