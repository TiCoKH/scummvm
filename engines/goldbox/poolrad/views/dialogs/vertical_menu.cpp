...
void VerticalMenu::updateHorizontalMenu() {
    // Remove any previously inserted dynamic items (Next/Prev, and Exit if auto-managed).
    for (int i = (int)_hMenuList.items.size() - 1; i >= 0; --i) {
        char sc = _hMenuList.items[i].shortcut;
        if (sc == 'N' || sc == 'P' || (_addExit && sc == 'E'))
            _hMenuList.items.remove_at(i);
    }

    // Append Next/Prev if needed
    if (_linesBelow > 0) {
        _hMenuList.items.push_back(MenuItem());
        int idx = (int)_hMenuList.items.size() - 1;
        _hMenuList.items[idx].text = "Next";
        _hMenuList.items[idx].active = true;
        _hMenuList.items[idx].shortcutFirst = true;
        _hMenuList.generateShortcut(idx);
    }
    if (_selectMin < _linesAbove) {
        _hMenuList.items.push_back(MenuItem());
        int idx = (int)_hMenuList.items.size() - 1;
        _hMenuList.items[idx].text = "Prev";
        _hMenuList.items[idx].active = true;
        _hMenuList.items[idx].shortcutFirst = true;
        _hMenuList.generateShortcut(idx);
    }

    // Always append Exit last when _addExit is set
    if (_addExit) {
        _hMenuList.items.push_back(MenuItem());
        int idx = (int)_hMenuList.items.size() - 1;
        _hMenuList.items[idx].text = "Exit";
        _hMenuList.items[idx].active = true;
        _hMenuList.items[idx].shortcutFirst = true;
        _hMenuList.generateShortcut(idx);
    }
...
void VerticalMenu::handleMenuResult(const MenuResultMessage &result) {
    ...
    // Navigation was handled — reactivate horizontal menu for next input.
    activateHorizontalMenu();
